...
class THMT {
    ...
    function parseDegreeTable($link) {
        ...
        foreach($html('table tr') as $ele) {
            ...
            $degree1ID   =  iconv("ISO-8859-1//IGNORE", "UTF-8//IGNORE", $degree1ID);
            $degree1Name = iconv("ISO-8859-1//IGNORE", "UTF-8//IGNORE", $degree1Name);
            $degree2ID   = iconv("ISO-8859-1//IGNORE", "UTF-8//IGNORE", $degree2ID);
            $degree2Name = iconv("ISO-8859-1//IGNORE", "UTF-8//IGNORE", $degree2Name);

            /*echo '--------parseDegreeTable<br>';
            echo $degree1ID;
            echo ' - ';
            echo $degree1Name;
            echo '<br>';
    ...
    function parseTimetable($timetableID) {
        //$html = file_get_dom('http://localhost:8888/do/'.$timetableID.'.html');
        ...
        foreach($html('table tr:odd') as $i1 => $ele) {
            ...
            foreach($ele('td') as $i2 => $tdEle) {
                ...
                if(strlen($tdEle->getPlainText()) > 0) {
                    ...
                    if($tdEle->bgcolor == '#80FFFF') $occurrence = 'odd';
                    else if($tdEle->bgcolor == '#FFFF80') $occurrence = 'even';
